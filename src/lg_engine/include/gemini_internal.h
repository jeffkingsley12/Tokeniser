...
static inline void atomic_store_float_relaxed(_Atomic uint32_t *p, float v) {
    uint32_t bits; 
    memcpy(&bits, &v, 4); 
    atomic_store_explicit(p, bits, memory_order_relaxed);
}
 ...
 * * Logs a warning if livelock ceiling is hit (indicates systemic contention).
 */
static inline void atomic_fetch_add_float(_Atomic uint32_t* p, float val) {
    /* Validate input to prevent NaN/Inf corruption */
    if (!isfinite(val)) return;
    
    uint32_t old_bits = atomic_load_explicit(p, memory_order_relaxed);
    uint32_t new_bits;
    float old_val, new_val;
    /* L-1 FIX: Separate loop iteration count from sched_yield call count.
     * The old code incremented `backoff_yields` on every CAS iteration,
     * including the fast-path where no yield occurs. This inflated the counter,
     * triggering the exponential backoff phase far too early under low contention.
     * `iterations` now counts all CAS attempts; `yields` counts only actual
     * sched_yield() calls, which is the right metric for the backoff threshold. */
    ...
    do {
        iterations++;
        /* L-1 FIX (corrected): The original fix separated `iterations` from
         * `yields`, but accidentally left the threshold guard on `yields`
         * instead of `iterations`.  Since `yields` starts at 0 and is only
         * incremented inside this block, the condition was permanently false
         * and the backoff never fired.  Use `iterations` as the trigger. */
        if (iterations > BACKOFF_THRESHOLD) {
            /* Exponential backoff: 1, 2, 4, 8, … yields per iteration */
            ...
            if (yields >= MAX_YIELDS) {
                fprintf(stderr,
                        "WARNING: atomic_fetch_add_float livelock detected at %p "
                        "(val=%f, iterations=%u)\n",
                        (void*)p, val, iterations);
                return;
            }
        }
 ...
 */
typedef struct {
    _Atomic uint32_t scc_id;  /* Promoted to atomic for thread-safe concurrent reads (CRITICAL FIX) */
    _Atomic uint32_t next_in_scc;  /* Promoted to atomic for thread-safe concurrent traversal (CRITICAL FIX) */
    _Atomic uint64_t freq;
    EdgeSpan in_edges;  /* PHASE 2A: Pooled adjacency descriptor (was: uint32_t* + first_in_edge) */
} NodeTransientState;
 ...
 * Contains aggregated metrics for promotion decisions
 */
typedef struct {
    NodeID head;            /* First node in SCC */
    NodeID tail;            /* Last node in SCC */
    _Atomic uint32_t member_count;  /* Number of nodes in SCC - CRITICAL FIX: Made atomic for thread-safe concurrent reads */
    _Atomic SccEdgeID first_scc_edge; /* Head of SCC-DAG adjacency list (CAS updates in le_add_scc_edge) */
    
    /* Structural metrics - CRITICAL FIX: Made atomic for thread-safe concurrent le_add_edge calls */
    ...
    /* Usage metrics - CRITICAL FIX: Made atomic for thread-safe concurrent le_add_edge calls */
    ...
    SymbolID symbol_id;
    bool is_candidate;
    bool is_weak;           /* Hairball detection */
    bool is_forced;         /* Deterministic transition (H -> 0) */
} SccNode;

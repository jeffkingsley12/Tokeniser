...
static void mark_scc_forced(EngineContext *ctx, NodeID n) {
    ...
    /* FIX H-4: SccNode.is_forced is a plain bool read by the engine thread
     * concurrently. A plain write is a C11 data race → UB. Use
     * __atomic_store_n with release ordering so the engine's acquire-load
     * of is_forced in le_process_epoch_stability sees the updated value.
     * TODO: promote is_forced to _Atomic bool in gemini_internal.h for full
     * portability (GCC __atomic builtins work on bool but are non-standard). */
    __atomic_store_n(&ctx->scc_nodes[sid].is_forced, true, __ATOMIC_RELEASE);
}

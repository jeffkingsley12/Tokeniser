...
static int inject_phrase(EngineContext *ctx, Tokenizer *tok,
    ...
    /* M-6 FIX: If weight_calls is very large (e.g. base_weight=1e10 * bonus_multiplier=2.0),
     * the cast (int)(weight_calls + 0.5f) is UB when the value exceeds INT_MAX.
     * Clamp before casting. A practical ceiling of 10000 calls is more than
     * sufficient for phrase seeding and prevents runaway edge weight inflation. */
    if (calls < 1) calls = 1;
    if (calls > 10000) calls = 10000;
...
static void mark_scc_forced(EngineContext *ctx, NodeID n) {
    ...
    ctx->scc_nodes[sid].is_forced = true;
}
